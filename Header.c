...
//This function produces a random number between a and b, inclusive. via https://www.geeksforgeeks.org/generating-random-number-range-c/
int rand_int(int a, int b)
{
	//srand() function prevents the repetition of the same random number after each execution of the program
	srand(time(0));
    return rand() % (b - a + 1) + a;
}

//This function generates three random numbers separately for slot machine.
...
//delay() function stops the execution for the time passed as argument in seconds. via https://www.geeksforgeeks.org/time-delay-c/
...
//This function that prints a string character by character with the delay passed to the function in seconds
...
//Function used to simulate the rolling of the ball on the Roulette table and landing on a random number
int spin()
{
	srand(time(0));
	int num = (rand() % 36) + 1;
...
//Function that is called when the user wants to play Craps
...
//Function that is called when the user wants to buy chips
void buy_x(int x, gamble* data)
{
	//Checks whether the user has enough amount to buy chips
	if(data -> amt < (x * 11))
	{
...
{
	//Checks whether user has enough chips to sell
	if(x > data -> chip || x <= 0)
	{
...
//Function that is called when user has to check balance
...
//Function that is called when the user wants to play Roulette
...
//Function that is called when the user wants to play Slot Machine
void PlaySlotMachine(gamble* data)
{
	...
	{
		print_str("Please enter an integer greater than 0 and lessthan 11 \n", 0.05);
